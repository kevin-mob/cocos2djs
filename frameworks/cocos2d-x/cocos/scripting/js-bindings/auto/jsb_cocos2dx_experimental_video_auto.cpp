...
void js_register_cocos2dx_experimental_video_VideoPlayer(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("getVideoDuration", js_cocos2dx_experimental_video_VideoPlayer_getVideoDuration, 0, JSPROP_PERMANENT |JSPROP_ENUMERATE),
        JS_FN("getVideoCurrentTime", js_cocos2dx_experimental_video_VideoPlayer_getVideoCurrentTime, 0, JSPROP_PERMANENT |JSPROP_ENUMERATE),
        JS_FN("setBrightness", js_cocos2dx_experimental_video_VideoPlayer_setBrightness, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FN("setVolume", js_cocos2dx_experimental_video_VideoPlayer_setVolume, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
...
bool js_cocos2dx_experimental_video_VideoPlayer_getVideoDuration(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
...
bool js_cocos2dx_experimental_video_VideoPlayer_getVideoCurrentTime(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    ...
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_experimental_video_VideoPlayer_isPlaying : Invalid Native Object");
    if (argc == 0) {
        int ret = cobj->getVideoCurrentTime();
        jsval jsret = JSVAL_NULL;
        jsret = INT_TO_JSVAL(ret);
        args.rval().set(jsret);
        return true;
...
bool js_cocos2dx_experimental_video_VideoPlayer_setVolume(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
...
bool js_cocos2dx_experimental_video_VideoPlayer_setBrightness(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
