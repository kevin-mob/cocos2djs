	...
	var ScaleTimeline = (function (_super) {
		...
		ScaleTimeline.prototype.apply = function (skeleton, lastTime, time, events, alpha, pose, direction) {
			...
			if (time < frames[0]) {
				switch (pose) {
					case MixPose.setup:
						bone.scaleX = bone.data.scalePointX;
						bone.scaleY = bone.data.scalePointY;
						return;
					case MixPose.current:
						bone.scaleX += (bone.data.scalePointX - bone.scalePointX) * alpha;
						bone.scaleY += (bone.data.scalePointY - bone.scalePointY) * alpha;
				}
				return;
			}
			var x = 0, y = 0;
			if (time >= frames[frames.length - ScaleTimeline.ENTRIES]) {
				x = frames[frames.length + ScaleTimeline.PREV_X] * bone.data.scalePointX;
				y = frames[frames.length + ScaleTimeline.PREV_Y] * bone.data.scalePointY;
			}
			else {
				...
				x = (x + (frames[frame + ScaleTimeline.X] - x) * percent) * bone.data.scalePointX;
				y = (y + (frames[frame + ScaleTimeline.Y] - y) * percent) * bone.data.scalePointY;
			}
			if (alpha == 1) {
				bone.scaleX = x;
				bone.scaleY = y;
				...
				var bx = 0, by = 0;
				if (pose == MixPose.setup) {
					bx = bone.data.scalePointX;
					by = bone.data.scalePointY;
				}
				else {
					bx = bone.scalePointX;
					by = bone.scalePointY;
...
(function (spine) {
	var Bone = (function () {
		...
		Bone.prototype.update = function () {
			this.updateWorldTransformWith(this.x, this.y, this.rotation, this.scalePointX, this.scalePointY, this.shearX, this.shearY);
		};
	...
	var Skeleton = (function () {
		...
		Skeleton.prototype.updateWorldTransform = function () {
			...
			for (var i = 0, n = updateCacheReset.length; i < n; i++) {
				...
				bone.ascaleX = bone.scalePointX;
				bone.ascaleY = bone.scalePointY;
...
(function (spine) {
	var RegionAttachment = (function (_super) {
		...
		RegionAttachment.prototype.updateOffset = function () {
			var regionScaleX = this.width / this.region.originalWidth * this.scalePointX;
			var regionScaleY = this.height / this.region.originalHeight * this.scalePointY;
			var localX = -this.width / 2 * this.scalePointX + this.region.offsetX * regionScaleX;
			var localY = -this.height / 2 * this.scalePointY + this.region.offsetY * regionScaleY;
